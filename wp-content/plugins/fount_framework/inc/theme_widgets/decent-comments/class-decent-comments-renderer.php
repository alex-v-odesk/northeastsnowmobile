...
class Decent_Comments_Renderer {
	...
	static function get_comments( $options = array() ) {
		...
		include_once( plugin_dir_path( __FILE__ ) . '/class-decent-comment.php' );
		...
		if ( !empty( $comments ) ) {
			...
			$output .= '<div class="comments_slider decent-comments per_init" data-autoplay="true" data-delay="6000">';
			...
			foreach ( $comments as $comment) {
				...
				if ( $show_comment ) {
					$output .= '<span class="prk_bordered prk_speech comment-' . ( $excerpt ? "excerpt" : "body" ) . '">';
					$output .= '<div class="prk_bordered prk_inner_tip"></div><div class="tip_top_hide"></div>';
					$output .= self::get_comment( $comment, array( "ellipsis" => $ellipsis, "excerpt" => $excerpt, "max_excerpt_words" => $max_excerpt_words) );
					$output .= '</span>'; 
				}
				
				if ( $show_avatar ) {
					$output .= '<span class="comment-avatar fade_anchor">';
				...
				if ( $show_author ) {
					$output .= '<span class="comment-author default_color fade_anchor prk_heavier_600 header_font">';
