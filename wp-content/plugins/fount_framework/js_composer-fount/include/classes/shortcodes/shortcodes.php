	...
	abstract class WPBakeryShortCode_UniversalAdmin extends WPBakeryShortCode {
		...
		public function contentAdmin( $atts, $content ) {
			...
			if ( isset( $this->settings['params'] ) ) {
				...
				//PIRENKO 
                $modal_prk=sanitize_title(htmlspecialchars(__($this->settings['name'], "js_composer")));
                $output .= '<div class="prkadmin-'.$modal_prk.' '.implode(' ', $editor_css_classes).'" data-title="'.htmlspecialchars(__('Edit', 'js_composer').' '.__($this->settings['name'], "js_composer")).'">';
                //END PIRENKO
...
if ( ! class_exists( 'WPBakeryShortCodeFishBones' ) ) {
	class WPBakeryShortCodeFishBones extends WPBakeryShortCode {
		...
		public function shortcodeClass() {
			...
			$class_name = 'WPBakeryShortCode_' . $this->settings( 'base' );
